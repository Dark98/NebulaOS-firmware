# klippy_extras/ module layout — design sketch, not yet implemented
...
## Not decided yet / worth a second look before implementing

1. Confirm the "keep Creality's exact names" call above - it's a recommendation, not yet agreed.
2. `Z_OFFSET_AUTO` vs `Z_OFFSET_CALIBRATION` - `z_compensate_wrapper.so`'s strings show both exist
   as separate registered commands (`cmd_Z_OFFSET_AUTO` and `cmd_Z_OFFSET_CALIBRATION`) but no
   macro in this printer's config actually calls `Z_OFFSET_AUTO` - only `Z_OFFSET_CALIBRATION` is
   used (via `custom_macro.py` and the `ZZ_OFFSET_TEST` manual test macro). Might be safe to skip
   `Z_OFFSET_AUTO` entirely for v1, or stub it as an alias - needs a decision, not a research gap.
3. Exact retry/error-code surface for `touch_probe()` - how closely to mirror Creality's specific
   `PR_ERR_CODE_*` catalog (ANALYSIS.md §1) vs. just raising plain `command_error` with a clear
   message. Leaning toward the latter for a clean rewrite, but worth confirming.
