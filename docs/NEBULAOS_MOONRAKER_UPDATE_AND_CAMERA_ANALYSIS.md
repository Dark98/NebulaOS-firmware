...
**Open, unresolved anomaly - not root-caused this session.** During this qualification the seeded camera's `service` field was found to have changed from its correct seeded value (`mjpegstreamer-adaptive`) to `uv4l-mjpeg` in the Moonraker database (confirmed via direct `sqlite3` inspection of `namespace_store`, not just the API - so this was a real persisted value change, not a display-layer artifact). `vendor/moonraker/moonraker/components/webcam.py` has no code path that would produce this on its own (`from_database()`/`as_dict()` are pure passthroughs, `component_init()` only reads from the database except in a narrow UID-collision case) - the value must have been written by an actual API call, but `moonraker.log` shows no access-log trace of any `/server/webcams/item` request during the relevant window (confirmed access logging for this endpoint category is simply not present in this log, not that no call happened). Restored via the supported `POST /server/webcams/item` API (not a direct database edit). Not reproduced a second time during the rest of this session's extensive Klipper/Moonraker restart cycles. Flagged here rather than claimed fixed - if this recurs during the post-rebuild qualification pass, it needs a dedicated investigation (candidates not yet checked: GuppyScreen's own webcam-related startup behavior, a klippy_extras identify/webhooks handshake).

**Qualification gate for the final tag.** Bug 6's fix and Bug 7's fix are both committed in source but **not yet baked into a rebuilt, reflashed image** - the live device this session used remains on its Phase O image (commit `9491085`) with the underlying bugs manually worked around in place (old init.d scripts still present but confirmed not to have altered the correct current state; moonraker manually kept alive across the Recover/Rollback tests). A full clean rebuild, reflash to the spare slot, and a fresh from-scratch offline-first-boot re-qualification (confirming Bug 6's fix actually removes the stale scripts from the shipped squashfs, and Bug 7's fix is present and does not need manual intervention) is required before the final tag - tracked separately in this document's build/flash record.
