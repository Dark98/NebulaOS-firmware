...
## Fix implemented, validated live (two build cycles)
...
**First flash of this fix surfaced a real, caught-and-fixed regression**: removing the DAI links
left six `audio_map[]` DAPM routes dangling (they terminated at `"DMIC capture"`/`"BAIC4
playback"`/`"BAIC4 capture"` - widgets that were auto-generated by the now-removed DAI links, not
statically declared). Confirmed live: `"ASoC: Failed to add route DMIC capture(*) -> LI0"`,
`snd_soc_register_card failed -19` - a different failure than the original `-517`, but
`/proc/asound/cards` still showed no soundcards registering. Fixed in a follow-up commit
(`cc86e471d`) that removed exactly those six routes, leaving the fixed hardware mixer ports
(`LI0`/`LI6`/`LO4_MUX`/`LO5_MUX`/`LO9_MUX`) and statically-declared widgets (`DMIC`/`PCMIN`/`PCMOUT`)
untouched - they're valid regardless of whether anything feeds them.

**Final validated result** (rebuilt, reflashed, rebooted with both commits applied):

| Check | Before this investigation | After |
|---|---|---|
| `/proc/asound/cards` | `--- no soundcards ---` | **`0 [halley5v30]: halley5_v30 - halley5_v30`** - a real ALSA card registers, for the first time in this project |
| `/proc/asound/pcm` | (empty) | All 10 `DMA0`-`DMA9` playback/capture streams enumerated |
| `dmesg`: `ASoC: CPU DAI DMIC not registered` / `snd_soc_register_card failed` | Present every boot | **Gone entirely** |
| `dmesg`: `gpio functions has redefinition` / `GP:GPD` backtrace | Present every boot | **Gone entirely** (`dmesg \| grep -c` → `0`) |
| `wlan-reg-on` (`GPD-4`) | Working, but racing `baic4_pd`'s claim | Still working, cleanly: `WLAN_REG_ON GPIO acquired, direction_output_raw(0) ret=0`, `gpio-100 (wlan-reg-on) out hi` |
| WiFi firmware load | Working | Still working: `BCM43430/1 ... version 7.46.58.13` |
| Beeper (`guppybeep tone 1000 300`) | Broken (missing `/usr/data/guppyscreen` path) | Fixed and confirmed: exit code `0`, path resolves via the `S01tmpfs-datastore` symlink baked into this same image |
| `invalid GPIO` warnings | `0` (already fixed in the prior boot-cleanup cycle) | Still `0` |
| Moonraker | Healthy | Still healthy (`klippy_connected: true`) |
